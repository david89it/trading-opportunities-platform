...
export const EquityCurveChart: React.FC<EquityCurveChartProps> = ({
  ...
  const processChartData = useMemo(() => {
    ...
    for (let i = 0; i <= totalTrades; i++) {
      const valuesAtPoint: number[] = [];
      
      // Collect values at this time point from all paths
      simulationResults.equity_paths.forEach(path => {
        const value = path[i];
        if (typeof value === 'number') {
          valuesAtPoint.push(value);
        }
      });
      
      if (valuesAtPoint.length > 0) {
        valuesAtPoint.sort((a, b) => (a || 0) - (b || 0));
        
        const validValues = valuesAtPoint.filter(v => typeof v === 'number');
        const mean = validValues.reduce((sum, val) => (sum || 0) + (val || 0), 0) / validValues.length;
        const p25Index = Math.floor(validValues.length * 0.25);
        const p75Index = Math.floor(validValues.length * 0.75);
        
        meanPath.push(mean);
        p25Path.push(validValues[p25Index] ?? mean);
        p75Path.push(validValues[p75Index] ?? mean);
      }
    }
    ...
    individualPaths.forEach((path, index) => {
      ...
      if (series) {
        const data: LineData[] = path.map((equity, i) => ({
          time: i as any, // Using time as trade number
          value: equity,
        }));
        series.setData(data);
      }
